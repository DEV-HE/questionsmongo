...
class ChatwootClass {
    ...
    formatNumber = (number) => {
        ...
        if (typeof number !== 'string') {
            // Si number no es una cadena, intenta convertirlo a cadena
    ...
    createContact = async (dataIn = { from: "", name: "", inbox: "" }) => {
        try {
            ...
            // Verificar si la respuesta contiene la propiedad 'contact'
            if (response && response.payload && response.payload.contact) {
                return response.payload.contact;
            } else {
                // Si la respuesta no contiene 'contact', verificar si contiene un mensaje de error específico
                if (response && response.message === 'Phone number has already been taken') {
                    console.error(`[Error createContact] Phone number has already been taken`);
                } else {
                    console.error(`[Error createContact] Response does not contain expected 'contact' property:`, response);
                }
                return null;
            }
        } catch (error) {
